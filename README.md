...
**Phase 0 — Discovery / Analyst Pack** завершена: канонические границы, архитектура, ADR, стандарты документации, слои Cursor и каркас каталогов согласованы с `docs/STRUCTURE.md` (включая placeholder-файлы `.gitkeep` там, где исходный код ещё не добавлен).

Следующий шаг — **Phase 1 — MVP Structured Import**: выбрать первый срез реализации (см. `tasks/current-task.md` и `MANIFEST.md`) и начать контракты/адаптеры без расширения scope за пределы MVP.
