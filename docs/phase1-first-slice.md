# Phase 1 — First Implementation Slice (MusicXML)

## Selected slice

**End-to-end deterministic path:** `MusicXML file → canonical score → arrangement plan → REAPER-oriented scaffold bundle (JSON) → review report.**

Primary structured input: **MusicXML 3.x** (partwise), starting with minimal single-part scores.
...
- Canonical model can be populated from at least one real interchange format.
...
- Guitar Pro / MIDI / manual tab ingestion.
...
| `src/mapping/` | Canonical → layout plan | `CanonicalProject` | `ArrangementPlan` | One `LayoutTrack` per canonical track | Smart roles, sections, stems |
